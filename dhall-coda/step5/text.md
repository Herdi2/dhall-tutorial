...
Go ahead and try that with our given `users.json` and see what happens:
```
json-to-dhall --file users.json --output users.dhall
batcat -l users.dhall
```{{execute}}
...
Think of this project, with incorrect `users.json`, as an alternate timeline where the DevOps course was bad (I know, hard to believe!).
...
Take a look at the schema for `users.json`:
```
batcat -l schema.dhall
```
...
```
json-to-dhall ./schema.dhall --file users.json --output users.dhall
```{{execute}}
Since the `users.json` was incorrect, as noted before, we get errors.
...
In this alternate timeline where Dhall was not used, `users.yaml` was written by hand!
Take a look:
```
batcat -l users.yaml
```{{execute}}
...
```
json-to-dhall ./schema.dhall --file users.json --output users_json.dhall
yaml-to-dhall ./schema.dhall --file users.yaml --output users_yaml.dhall
diff -w users_json.dhall users_yaml.dhall
```{{execute}}
...
### Verification
The verification will make sure that a `users.dhall` exists, and that `users_json.dhall` and `users_yaml.dhall` exist.
It will also make sure that there are no errors in `users.json`.
